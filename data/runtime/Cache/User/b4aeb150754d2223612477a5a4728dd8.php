<?php if (!defined('THINK_PATH')) exit();?><!doctype html>
<html>
<head>
	<meta charset="utf-8">
	<!-- Set render engine for 360 browser -->
	<meta name="renderer" content="webkit">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <!-- HTML5 shim for IE8 support of HTML5 elements -->
    <!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
    <![endif]-->

	<link href="/info/statics/simpleboot/themes/<?php echo C('SP_ADMIN_STYLE');?>/theme.min.css" rel="stylesheet">
    <link href="/info/statics/simpleboot/css/simplebootadmin.css" rel="stylesheet">
    <link href="/info/statics/js/artDialog/skins/default.css" rel="stylesheet" />
    <link href="/info/statics/simpleboot/font-awesome/4.2.0/css/font-awesome.min.css"  rel="stylesheet" type="text/css">
    <style>
		.length_3{width: 180px;}
		form .input-order{margin-bottom: 0px;padding:3px;width:40px;}
		.table-actions{margin-top: 5px; margin-bottom: 5px;padding:0px;}
		.table-list{margin-bottom: 0px;}
	</style>
	<!--[if IE 7]>
	<link rel="stylesheet" href="/info/statics/simpleboot/font-awesome/4.2.0/css/font-awesome-ie7.min.css">
	<![endif]-->
<script type="text/javascript">
//全局变量
var GV = {
    DIMAUB: "/info/",
    JS_ROOT: "statics/js/",
    TOKEN: ""
};
</script>
<!-- Le javascript
    ================================================== -->
    <!-- Placed at the end of the document so the pages load faster -->
    <script src="/info/statics/js/jquery.js"></script>
    <script src="/info/statics/js/wind.js"></script>
    <script src="/info/statics/simpleboot/bootstrap/js/bootstrap.min.js"></script>
<?php if(APP_DEBUG): ?><style>
		#think_page_trace_open{
			z-index:9999;
		}
	</style><?php endif; ?>
<body class="J_scroll_fixed">
	<div class="wrap J_check_wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="javascript:;">测评记录</a></li>
		</ul>
		<form class="well form-search">
			<div class="search_type cc mb10">
				<div class="mb10">
					<span class="mr20">宝宝名称：</span><input type="text" id="Baby_Name" value="<?php echo ($Baby_Name); ?>" />
					
                    宝宝生日：
                    <input type="text" class="J_date date" value="<?php echo ($Baby_Date); ?>" id="Baby_Date">
                    所属
                    <select id="type" style="max-width:60px">
                    <option value="0">手机</option>
                    <option value="1" <?php if($type == 1): ?>selected<?php endif; ?>>pad</option>
                    </select>
                    测评时间:<input type="text" value="<?php echo ($begin); ?>" class="J_date J_datetime" id="begin">
                    -
                    <input type="text" value="<?php echo ($end); ?>" class="J_date J_datetime" id="end">
                    <a href="javascript:;" onClick="todo()" class="btn btn-primary">搜索</a>
				  <script>
					function todo() {
						var Baby_Name = $("#Baby_Name").val();
						var Baby_Date = $("#Baby_Date").val();
						var begin = $("#begin").val();
						var end = $("#end").val();
						var type = $("#type").val();
						window.location.href='index.php?g=User&m=Game&a=babyHistory&menuid=185&Baby_Name='+Baby_Name+'&Baby_Date='+Baby_Date+'&begin='+begin+'&end='+end+'&type='+type;
					}
					</script>
				</div>
			</div>
		</form>
		<form class="form-horizontal J_ajaxForm" action="" method="post">
			<table class="table table-hover table-bordered">
				<thead>
					<tr>
						<th width="100">ID</th>
						<th>适用月龄</th>
                        <th>宝宝名称</th>
                        <th>身高</th>
                        <th>体重</th>
                      <th>头围</th>
                      <th>BMI</th>
                      <th>总分</th>
                      <th>各项得分</th>
                        <th width="120">操作</th>
					</tr>
				</thead>
				<?php if(is_array($result)): foreach($result as $key=>$vo): ?><tr valign="middle" >
					<td><?php echo ($vo["id"]); ?></td>
					<td><?php echo ($vo["month"]); ?></td>
                    <th><?php echo ($vo["baby_name"]); ?></th>
                    <td><?php echo ($vo["height"]); ?></td>
                    <th>
                    <?php echo ($vo["weight"]); ?>
                  </th>
                  <td><?php echo ($vo["header"]); ?></td>
                  <td><?php echo ($vo["bmi"]); ?></td>
                  <td><?php echo ($vo["total"]); ?></td>
                  <td>
                  <?php $score = unserialize($vo['score']); ?>
                  <?php if(is_array($score)): foreach($score as $key=>$v): ?>分类：<?php echo ($v["term_name"]); ?> | 得分：<?php echo sprintf("%0.2f",$v['score']);?><br/><?php endforeach; endif; ?>
                  
                  </td>
                    <td>
                    <a href="<?php echo U('Game/babyDo',array('id'=>$vo['id']));?>" target="_blank">查看详细</a>
					</td>
				</tr><?php endforeach; endif; ?>
			</table>
		</form>
        <div class="pagination"><?php echo ($page->show('Admin')); ?></div>
	</div>
	<script src="/info/statics/js/common.js"></script>
	<script>
		$(function() {

			$("#navcid_select").change(function() {
				$("#mainform").submit();
			});

		});
	</script>
</body>
</html>