...
<script src="tpl_admin/simpleboot/User/Game/echarts.min.js"></script>
	...
	<div class="wrap J_check_wrap">
        ...
        <li><a href="<?php echo U('Wchat/activeDay');?>">日活信息</a></li>
			<li class="active"><a href="<?php echo U('Wchat/activeMonth');?>">月活信息</a></li>
		</ul>
        <form class="well form-search">
			<div class="search_type cc mb10">
				<div class="mb10">
                    ...
                    <?php for ($i=$ago;$i<=date("Y");$i++) { ?>
                    <option value="<?php echo ($i); ?>" <?php if($begin == $i): ?>selected<?php endif; ?>><?php echo ($i); ?>年</option>
                    <?php } ?>
                    </select>
						结束时间：<select id="end" style="width:120px">
                        <option value="0">请选择</option>
                    <?php for ($i=$ago;$i<=date("Y");$i++) { ?>
                    <option value="<?php echo ($i); ?>" <?php if($end == $i): ?>selected<?php endif; ?>><?php echo ($i); ?>年</option>
                    <?php } ?>
                    </select>
					</span>
